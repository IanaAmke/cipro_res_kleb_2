...
for seq_record in SeqIO.parse("filtered_mutations_parc_aa_seq.faa", "fasta"):
    header = str(seq_record.id)
    sequence = str(seq_record.seq).upper()
    fasta_name=header.split(" ")[0]

    with open("kleb_pneumo_parc_blast_mismatch.tsv","r") as blast_output:
        ...
        for line in mismatch:
            ...
            if line[1] == fasta_name:
                if fasta_name not in aa_seq:
                    with open("parc_mismatch.fasta","a") as new_file:
