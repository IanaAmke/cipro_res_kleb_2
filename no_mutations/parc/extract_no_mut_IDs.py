	...
	for seq_record in SeqIO.parse(file, "fasta"):
		...
		with open("cipro_res_no_mutations.tsv", "r") as no_mut:
			...
			for line in no_mut_file:
				cipro_no_mut_file.append(line[0])
				if line[0] == fasta1[0]:
					if fasta1[0] not in included_mutations:
						with open("filtered_mutations.fasta","a") as new_file:
							new_file.write(">" + line[0] + "_" + fasta1[1] + "\n" + sequence + "\n")
							included_mutations.append(fasta1[0])	

first_set = set(included_mutations)
second_set = set(cipro_no_mut_file)
print("The missing genes are:")
