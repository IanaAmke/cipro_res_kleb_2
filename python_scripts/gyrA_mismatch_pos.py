...
path_to_ref = 'gyra_ref_aa_seq.faa'   # Reference data
...
# File path to your FASTA file
path_to_sub = 'gyra_blast_align_with_ref.faa'   # Sequences aligned to the reference amino acid sequence
...
for _, record in records:
    ...
    for pos, (ref_str, actual_str) in enumerate(zip(ref, record)):
        if ref_str != actual_str:
            if actual_str != "-":
                mismatch[-1].append(f'GyrA-{pos + 1}{actual_str}{ref_str}')

with open(f"gyrA_truncations.csv", "w+") as file:
    file.write('Sequence ID, Sequence, Name\n')

with open(f"no_gyrA_mutations.csv", "w+") as file:
    file.write(f'Sequence ID, Sequence, Name\n')
    
with open(f"gyrA_mutations.csv", "w+") as file:
