...
# group fluoroquinolone resistance determinants
...
## Summary bar plots of sample genomes being analysed {.tabset .tabset-pills}
...
### Summary bar plot of resistance determinants per phenotype {.tabset .tabset-pills}
#### Count table
```{r}
...
summ_res_mech %>%
  ggplot(aes(Resistance.mech.type, count)) +
  geom_bar(stat = "identity", fill = "#00ff2f", alpha = 0.6) +
  facet_wrap(~ Resistance.phenotype, ncol = 3) +
  geom_text(aes(label = count), stat = "identity", nudge_y = 70) +
...
####
From the violin plots, one can see that there are genomes with no mutations, 1299/10448 (12.43%), that are resistant to ciprofloxacin. Those genomes were filtered out and saved as **cipro_res_no_mutations.tsv**. Due to there being genomes that were resistant to ciprofloxacin but had no mutations in *gyrA* or *parC*, gene sequences of the two genes were extracted from the genomes, translated into proteins, and checked for mutations. 
...
Using seqkit, gaps and duplicated sequences were removed from **filtered_gyrA_parC.fasta** and thereafter, *gyrA* and *parC* gene sequences were separated to obtain **filtered_gyrA.fasta** and **filtered_parC.fasta** as output files:

```{bash eval=FALSE}
# remove gaps
seqkit seq -g filtered_gyrA_parC.fasta > filtered_gyrA_parC_nogaps.fasta

# check for duplicated sequences
seqkit rmdup -n < filtered_gyrA_parC_nogaps.fasta > filtered_gyrA_parC_nogaps_NR.fasta -D filtered_gyrA_parC_nogaps_NR_duplicate_info.txt 
-d filtered_gyrA_parC_nogaps_NR_duplicate_seqs.fasta

# separate gyrA and parC gene sequences
seqkit grep -r -p gyrA filtered_gyrA_parC_nogaps_NR.fasta -o filtered_gyrA.fasta
seqkit grep -r -p parC filtered_gyrA_parC_nogaps_NR.fasta -o filtered_parC.fasta
```

## Translation of nucleotide sequences
Prodigal was run to translate the *gyrA* and *parC* nucleotide sequences into amino acid sequences:

```{bash eval=FALSE}
prodigal -i filtered_gyrA.fasta -o filtered_gyrA_gene.coords.gbk -a filtered_gyrA_aa.faa
prodigal -i filtered_parC.fasta -o filtered_parC_gene.coords.gbk -a filtered_parC_aa.faa
```
...
Alignment of the genomes to the GyrA and ParC reference sequences was done to check whether the sample genome had mutations in those proteins. **parc_blast.faa** and **gyra_blast.faa** were first aligned to the reference sequences using mafft and the reference sequences were removed from the resulting alignment files: 

```{bash eval=FALSE}
# align gene sequences
mafft parc_blast.faa > parc_blast_align.faa
mafft gyra_blast.faa > gyra_blast_align.faa

# align sequences to the references
mafft --addfull parc_ref_aa_seq.faa parc_blast_align.faa > parc_blast_align_with_ref.faa
mafft --addfull gyra_ref_aa_seq.faa gyra_blast_align.faa > gyra_blast_align_with_ref.faa
```

Afterwards, strains with WT GyrA/ParC, those with mutations in the proteins and with truncated amino acid sequences were summarised using **gyrA_mismatch_pos.py** and **parC_mismatch_pos.py**. The mutations, obtained from **gyrA_mutations.csv** and **parC_mutations.csv**, were then compared to the Kleborate output and mutations not in Kleborate but fall within the quinolone resistance-determining region (QRDR) i.e. position 67-106 for GyrA and 63-102 for ParC were saved in **mutations_not_in_kleborate.tsv**. Mutations different from those picked up by Kleborate were marked with a double asterisk (**).
...
cipro_antibiogram %>%
  ...
  geom_bar(fill = "#ff8000", alpha = 0.6) +
  geom_text(aes(label = ..count..), stat = "count", nudge_y = 60) +
  labs(title = "Total sample count per dataset", y = "Total sample count") +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))
```

### Sample count per phenotype
```{r}
cipro_antibiogram %>%
  group_by(Resistance.phenotype) %>%
  ggplot(aes(x = Resistance.phenotype)) +
  geom_bar(fill = "#ff0000", alpha = 0.6) +
  geom_text(aes(label = ..count..), stat = "count", nudge_y = 80) +
  labs(title = "Total sample count for each phenotype", x = "Phenotype", y = "Total sample count") 
```
...
cipro_antibiogram %>%
  ...
  ggplot(aes(x = factor(Measurement), fill = index)) + geom_bar() + 
  # CLSI cut-off of 1.0 mg/L is resistant
  geom_vline(aes(xintercept = 4, linetype = "resistant"), alpha = 0.6, color = "black") +
  # CLSI cut-off of 0.25 mg/L is susceptible
  geom_vline(aes(xintercept = 2, linetype = "susceptible"), color = "black") +
  labs(title = "Total sample count at various MIC (broth dilution) measurements", x = "Measurement (mg/L)", y = "Frequency", fill = "Dataset", 
       linetype = "MIC breakpoints") +
  scale_linetype_manual(values = c("solid", "dotted")) 
```
...
cipro_res_mech <- cipro_antibiogram %>%
  pivot_longer(Flq_mutations:Flq_acquired, names_to = "Flq.resistance.mech", 
               values_to = "Resistance.mech.type") %>%
  separate(Resistance.mech.type, into = c("type_A", "type_B", "type_C", "type_D", "type_E", "type_F", "type_G", "type_H", "type_I"), sep = "[;]")

cipro_res_mech <- cipro_res_mech %>%
  pivot_longer(cols = names(cipro_res_mech)[26:34], values_to = "Resistance.mech.type") %>%
  drop_na(Resistance.mech.type) %>%
  select(-name) 
...
cipro_res_mech_prop <- cipro_antibiogram %>%
  separate(Flq_mutations, into = c("mut_A", "mut_B", "mut_C", "mut_D", "mut_E", "mut_F", "mut_G", "mut_H", "mut_I"), sep = "[;]") %>%
...
cipro_res_mech_prop <- cipro_res_mech_prop %>%
  ...
  group_res_mech(mut_D) %>%
  group_res_mech(mut_E) %>%
  group_res_mech(mut_F) %>%
  group_res_mech(mut_G) %>%
  group_res_mech(mut_H) %>%
  group_res_mech(mut_I) %>%
...
cipro_res_mech_prop <- cipro_res_mech_prop %>%
  unite("Flq_mutations", mut_A, mut_B, mut_C, mut_D, mut_E, mut_F, mut_G, mut_H, mut_I, sep = ";", na.rm = TRUE) %>%
...
labs = labs[labs %in% "functions"]
```
```{r all-code, ref.label=labs, eval=FALSE}
```

## Statistical analysis
```{r get-labels-2, echo = FALSE}
labs = knitr::all_labels()
labs = labs[labs %in% "proportion"]
```
```{r all-code-2, ref.label=labs, eval=FALSE}
```
