...
### Disk diffusion
```{r warning=FALSE, fig.width=20, fig.height=9}
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE,
                      dpi = 300, cache = FALSE, attr.output = 'style="max-height: 300px;"')
...
# run prop.test() on each row
prop_out <- dapply(slt(mech_freq, c(resistant, total)), MARGIN = 1, 
       FUN = function(x) broom::tidy(prop.test(x[1], x[2])))

# combine mech_freq and prop_out
mech_freq_prop <- cbind(mech_freq, prop_out)
