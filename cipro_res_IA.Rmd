...
# now have one row per strain/mechanism combination
view(cipro_res_mech)

# KH: now label all OmpK35 truncations as such, e.g. "OmpK35-21%" -> "OmpK35-trunc"
...
```{r echo=FALSE, fig.height=12, fig.width=55, warning=FALSE, cache=FALSE}
...
# Add more colors to this palette :
colour_palette_2 <- colorRampPalette(colour_palette_2)(26)
...
st_res_mech_freq <- st_mech %>%
  ...
  summarise(n = n()) %>%
  mutate(freq = n / sum(n)) %>%
  mutate(ST = factor(ST, levels = unique(st_res_mech_freq$ST), ordered = TRUE))
